...
export default function Terminal() {
  ...
  return (
    <Card className="h-full p-0 overflow-hidden" glow={true}>
      {/* CRT Scanline Effect Overlay */}
      <div className="crt-scanline pointer-events-none z-20"></div>

      <div className="h-full flex flex-col relative z-10">
        {/* Terminal Header */}
        <div className="flex justify-between items-center px-3 py-1 bg-white/5 border-b border-white/5 shrink-0">
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-2 font-mono text-[10px] space-y-0.5 relative"
        >
          {logs.length === 0 && (
          ...
          {logs.map((log, i) => (
            <div
              key={i}
              className="flex gap-3 hover:bg-white/5 p-0.5 rounded transition-colors group"
            >
